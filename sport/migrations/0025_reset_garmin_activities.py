...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
      # Re-Attach Garmin activities       
      activities = orm.GarminActivity.objects.all()
      nb = activities.count()
      for i, activity in enumerate(activities):
        print '%d/%d %s %s' % (i, nb, activity.sport.name, activity.garmin_id)

        # Reproduce Attach session here
        # we can't access it from the frozen model in orm
    ...
    def backwards(self, orm):
      raise RuntimeError('No backward migration for Garmin activities')
