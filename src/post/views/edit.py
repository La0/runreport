...
class PostSessionsView(PostWriterMixin, JsonResponseMixin, DetailView):
    ...
    def load_date_form(self):
        if 'switch' in self.request.POST:
            ...
            self.date = date(
                year=int(
                    self.request.POST['date_year']), month=int(
                    self.request.POST['date_month']), day=int(
                    self.request.POST['date_day']))
        else:
            # Defaults to today
    ...
    def load_user_sessions(self):
        ...
        sessions = sessions.filter(
            day__date__month=self.date.month,
            day__date__year=self.date.year)
