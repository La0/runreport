...
class PostView(DetailView):
    ...
    def get_object(self):
        ...
        post = get_object_or_404(
            Post,
            writer__username=self.kwargs['username'],
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        # Embed Medias
        context['medias'] = self.object.medias.filter(
            type='image crop').prefetch_related(
            'parent', 'post')

        # Embed sessions
        context['sessions'] = self.object.sessions.all().order_by(
            'day__date').prefetch_related('track', 'sport', 'day')
