...
class PaymentPeriodView(PaymentPeriodMixin):
    ...
    def post(self, *args, **kwargs):
        ...
        return HttpResponseRedirect(
            reverse('club-manage', args=(period.club.slug, )))
...
class PaymentPeriodExport(PaymentPeriodMixin):
    ...
    def render_to_response(self, context, *args, **kwargs):
        ...
        date_fmt = '%d/%m/%Y'
        resp['Content-Disposition'] = 'attachment; filename="Bill %s - %s - RunReport.pdf"' % (
            period.start.strftime(date_fmt), period.end.strftime(date_fmt))
