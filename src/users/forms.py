...
class UserPasswordForm(forms.Form):
    ...
    def clean_password_check(self):
        if self.cleaned_data.get('password_new', 'a') != self.cleaned_data.get(
                'password_check', 'b'):
    ...
    def clean_birthday(self):
        ...
        if diff.days < 13 * 365:
            raise forms.ValidationError(
                _('You must be at least 13 years old to create an account.'))
...
class GarminForm(forms.ModelForm):
    ...
    def clean_garmin_password(self):
        ...
        password = str(gpg.encrypt(self.clear_password, settings.GPG_KEY))
        if not password:
    ...
    def clean(self):
        ...
        try:
            ...
            provider.auth(
                force_login=self.cleaned_data['garmin_login'],
                force_password=self.clear_password)
