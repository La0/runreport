...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        days = [(start + timedelta(days=i)).date()
                for i in range(0, self.days_past + self.days_future)]
    ...
    def build_day(self, user, day):
        ...
        week, _ = SportWeek.objects.get_or_create(
            user=user, week=w, year=y, defaults={
                'published': published, })
    ...
    def build_session(self, sport_day):
        ...
        # special format for django orm (1 is sunday)
        week_day = int(sport_day.date.strftime('%w')) + 1
        rand_session = SportSession.objects.filter(
            name__isnull=False, day__date__week_day=week_day).order_by('?').first()
    ...
    def cleanup_links(self, club):
        ...
        # Add a link to ffa
        club.links.create(
            name=u'Fédération Athlé',
            url='http://www.athle.fr/index.aspx',
            position=0)
