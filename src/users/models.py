...
class Athlete(AthleteBase):
    ...
    frequency = models.IntegerField(
        ...
        validators=[
            MinValueValidator(0)])
    frequency_rest = models.IntegerField(
        _('cardiac frequency at rest'),
        null=True,
        blank=True,
        validators=[
            MinValueValidator(0)])
    height = models.IntegerField(
        _('height'), null=True, blank=True, validators=[
            MinValueValidator(0)], help_text=_('Unit: cm'))
    weight = models.IntegerField(
        _('weight'), null=True, blank=True, validators=[
            MinValueValidator(0)], help_text=_('Unit: kg'))
    comment = models.TextField(_('comment'), null=True, blank=True)
    phone = models.CharField(_('Phone'), max_length=50, null=True, blank=True)
    nb_sessions = models.IntegerField(
        _('number of sessions per week'),
        null=True,
        blank=True,
        validators=[
            MinValueValidator(0)])
    ...
    daily_trainer_mail = models.BooleanField(
        _('Daily trainer mail'), default=True)
    ...
    # Garmin
    garmin_login = models.CharField(
        _('garmin login'),
        max_length=255,
        null=True,
        blank=True)
    garmin_password = models.TextField(
        _('garmin password'), null=True, blank=True)
    ...
    # Profile privacy
    privacy_avatar = models.CharField(
        _('avatar visibility'),
        max_length=50,
        choices=PRIVACY_LEVELS,
        default='public')
    privacy_races = models.CharField(
        _('races visibility'),
        max_length=50,
        choices=PRIVACY_LEVELS,
        default='public')
    privacy_records = models.CharField(
        _('records visibility'),
        max_length=50,
        choices=PRIVACY_LEVELS,
        default='public')
    privacy_stats = models.CharField(
        _('stats visibility'),
        max_length=50,
        choices=PRIVACY_LEVELS,
        default='public')
    privacy_calendar = models.CharField(
        _('calendar visibility'),
        max_length=50,
        choices=PRIVACY_LEVELS,
        default='private')
    privacy_comments = models.CharField(
        _('comments visibility'),
        max_length=50,
        choices=PRIVACY_LEVELS,
        default='public')
    privacy_tracks = models.CharField(
        _('tracks visibility'),
        max_length=50,
        choices=PRIVACY_LEVELS,
        default='club')
    ...
    display_help = models.BooleanField(
        _('Display contextual help'), default=True)

    # Initial demo steps
    demo_trainer_active = models.BooleanField(default=True)
    demo_athlete_active = models.BooleanField(default=True)
    # json inside, should be JsonField on Django 1.9
    demo_steps = models.TextField(null=True, blank=True)

    def search_category(self):
        if not self.birthday:
            return None
        try:
            self.category = UserCategory.objects.get(
                min_year__gte=self.birthday.year,
                max_year__lte=self.birthday.year)
    ...
    def is_trainer_of(self, athlete):
        ...
        '''
        return athlete.memberships.filter(
            role__in=('athlete', 'trainer'), trainers=self).exists()
    ...
    def is_trained(self):
        ...
        return self.memberships.filter(
            role__in=('athlete', 'trainer'), trainers__isnull=False).exists()
    ...
    def get_visitor_rights(self, visitor):
        ...
        if visitor.is_authenticated():
            member_clubs = set([m['club__id'] for m in self.memberships.exclude(
                role__in=('prospect', 'archive')).values('club__id')])
            user_clubs = set([m['club__id'] for m in visitor.memberships.exclude(
                role__in=('prospect', 'archive')).values('club__id')])
    ...
    def local_privacy(self):
        ...
        fields = [k[8:] for k in dir(self) if k.startswith(
            'privacy')]  # all the current privacy fields
    ...
    def get_privacy_rights(self, visitor):
        ...
        if 'comments' in privacy:
            ...
            privacy += visitor == self and [
                'comments_private', 'comments_week', ] or []
    ...
    def unsubscribe_mailing(self, mailing, email=None):
        ...
        try:
            ...
            print(
                'Failed to unsubscribe %s from %s : %s' %
                (self.username, mailing, str(e)))
            return False
        return True
    ...
    def get_friend_status(self, friend):
        ...
        if FriendRequest.objects.filter(
                sender=self, recipient=friend).count() > 0:
            return 'request'
    ...
    def list_related_races(self):
        ...
        for club in self.club_set.all():
            members = club.clubmembership_set.exclude(
                role__in=('prospect', 'archive'))
    ...
    def check_demo_steps(self, mode, notify=True, force_steps=()):
        ...
        if mode == 'trainer':
            ...
            __add('plan_applied', self.plans.filter(
                applications__isnull=False).exists())
            ...
            __add(
                'gps',
                self.garmin_login is not None or self.strava_token is not None)
    ...
    def delete(self, *args, **kwargs):
        ...
        try:
            ...
            logger.error(
                'Failed to unsubscribe {} from mailing lists: {}'.format(
                    self, e))
