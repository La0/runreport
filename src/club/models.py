...
class Club(models.Model):
    ...
    slug = models.SlugField(
        _('Slug'),
        help_text="Represents the club's name in urls",
        unique=True,
        max_length=20)
    members = models.ManyToManyField(Athlete, through='ClubMembership')
    main_trainer = models.ForeignKey(
        Athlete,
        null=True,
        blank=True,
        related_name="club_main_trainer")
    ...
    # Private club ?
    private = models.BooleanField(_('Private club'), help_text=_(
        'When private, a club is only accessible y its members'), default=False)
    ...
    def load_usage_stats(self):
        ...
        plans = Plan.objects.filter(
            creator__memberships__club=self,
            creator__memberships__role='trainer').count()

        # Nb of plan applied for athletes
        applied = PlanApplied.objects.filter(
            user__memberships__club=self,
            user__memberships__role='athlete').count()

        # Nb of sessions
        sessions = SportSession.objects.filter(
            day__week__user__memberships__club=self).count()
...
class ClubInvite(models.Model):
    ...
    # a recipient user can already exist
    user = models.ForeignKey('users.Athlete', null=True, blank=True)
    ...
    slug = models.CharField(
        max_length=30,
        unique=True,
        blank=True)  # not a slug: no char restriction
    created = models.DateTimeField(auto_now_add=True)
    updated = models.DateTimeField(auto_now=True)
...
class ClubGroup(models.Model):
    ...
    def get_members(self):
        return self.members.all().order_by(
            'user__first_name', 'user__last_name').prefetch_related('user')
    ...
    def delete_mailing_list(self):
        ...
        try:
            ...
            print(
                'Failed to delete mailing list %s : %s' %
                (self.mailing_list, str(e)))
            return False
