...
class ClubList(ListView):
    ...
    def get_queryset(self):
        # List normal clubs
        club_ids = [
            c['pk'] for c in Club.objects.exclude(
                demo=True).exclude(
                private=True).values('pk')]

        # Add user's clubs
        if self.request.user.is_authenticated():
            club_ids += [c['club__pk']
                         for c in self.request.user.memberships.values('club__pk')]
...
class ClubJoin(JsonResponseMixin, TemplateView, ):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if not self.request.user.is_authenticated():
            self.request.session['club_join'] = (
                self.club.pk, self.kwargs.get('secret', None))
        ...
        member = ClubMembership.objects.create(
            club=self.club, user=self.request.user, role='prospect')
