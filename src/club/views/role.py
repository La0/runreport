...
class ClubMemberRole(JsonResponseMixin, ClubManagerMixin,
                     ModelFormMixin, ProcessFormView, DetailView):
        ...
        try:
            ...
            if membership.role == 'delete':
                ...
                if self.role_original != membership.role:
                    ...
                    trainers = self.club.clubmembership_set.filter(
                        role='trainer')
            ...
            logger.error(
                'Failed to save role update for %s : %s' %
                (membership.user, str(e)))
            raise
...
class ClubMemberTrainers(JsonResponseMixin, ClubManagerMixin,
                         ModelFormMixin, ProcessFormView, DetailView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            if not (membership.role ==
                    'athlete' and membership.trainers.count() == 0):
                self.json_options = [
                    JSON_OPTION_BODY_RELOAD,
                    JSON_OPTION_NO_HTML,
                    JSON_OPTION_CLOSE,
