...
class Command(BaseCommand):
        ...
        for i, day in enumerate(days):
            ...
            if day.sessions.count() >= 1:
                ...
                SportSession.objects.create(
                    ...
                    comment=day.comment,
                    type=day.type,
                    race_category=day.race_category)
    ...
    def match_sessions(self, day):
        ...
        # Debug
        #print('-' * 80)
        # print('Day #%d "%s" %s %s' % (day.pk, day.name, day.time,
        # day.distance))

        for session in day.sessions.all():
            # print('Session #%d "%s" %s %s %s' % (session.pk, session.name,
            # session.sport, session.time, session.distance))
