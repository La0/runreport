...
class CurrentWeekMixin(LoginRequired):
    ...
    def check_limits(self, check=True):
        ...
        olds = SportDay.objects.filter(
            week__user=self.get_user()).order_by('date')
        if olds.exists():
            oldest = olds.first()
    ...
    def get_maps_status(self):
        ...
        try:
            has_maps = Track.objects.filter(
                session__day__week=self.object,
                simple__isnull=False).exists()
...
class WeekPaginator(object):
    ...
    def paginate(self, page_date, min_date, max_date):
        ...
        current_pos = 0
        i = 0
        useful_weeks = [w for w in self.weeks if w['display']
                        == 'week']  # skip spacers
        for i, w in enumerate(useful_weeks):
            if w['start'] == page_date:
                current_pos = i
                break

        week_previous = current_pos - \
            1 > 0 and useful_weeks[current_pos - 1] or None
        week_next = current_pos + \
            1 < len(useful_weeks) and useful_weeks[current_pos + 1] or None
...
class CalendarSession(CalendarDay):
    ...
    def get_object(self):
        ...
        # Init a session
        if session_id:
            self.session = SportSession.objects.get(
                pk=session_id, day__week__user=self.request.user)
        else:
            if not self.object.pk:
                self.object.save()  # need a pk to attribute
            self.session = SportSession(
                sport=self.request.user.default_sport, day=self.object)
...
class AthleteRaces(object):
    ...
    def get_races(self, user):
        ...
        # List races days
        race_days = SportDay.objects.filter(
            week__user=user, sessions__type='race')
        ...
        cat_ids = [r['sessions__race_category'] for r in past_races.values(
            'sessions__race_category').distinct() if r['sessions__race_category']]
        categories = dict([(c.id, c) for c in RaceCategory.objects.filter(
            pk__in=cat_ids).order_by('name')])

        # Sum sessions time & distance per race
        past_races = past_races.annotate(
            time_total=Sum('sessions__time'),
            distance_total=Sum('sessions__distance'),
            nb_sessions=Count('sessions')).order_by('time_total')
    ...
    '''
        ...
        races = {}
        for r in past_races:
            for c in r.sessions.filter(type='race').values(
                    'race_category').distinct():
