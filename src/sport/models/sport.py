...
class SportSession(models.Model):
    ...
    # generated from potential markdown source above
    comment_html = models.TextField(null=True, blank=True)
    type = models.CharField(
        max_length=12,
        default='training',
        choices=SESSION_TYPES)
    race_category = models.ForeignKey(
        'RaceCategory',
    ...
    elevation_gain = models.FloatField(
        null=True, blank=True, verbose_name=_('Elevation gain'))
    elevation_loss = models.FloatField(
        null=True, blank=True, verbose_name=_('Elevation loss'))

    # Google Calendar
    gcal_id = models.CharField(max_length=255, null=True, blank=True)

    # Comments
    comments_public = models.OneToOneField(
        'messages.Conversation',
        null=True,
        blank=True,
        related_name='session_public')
    comments_private = models.OneToOneField(
        'messages.Conversation',
        null=True,
        blank=True,
        related_name='session_private')
    ...
    def save(self, *args, **kwargs):
        ...
        if self.sport.depth != 1:
            raise Exception(
                "Invalid sport '%s', only level 1 authorized for SportSession" %
                self.sport)
    ...
    def build_conversation(self, type):
        ...
        conversation = Conversation.objects.create(
            type=name, session_user=self.day.week.user)
