...
class PlanPdfExporter(object):
    ...
    def add_days(self):
        ...
        for i in range(1, 8):
            date = week_to_date(
                2015, 1, i %
                7)  # we just need the day name here
    ...
    def build_lines(self):
        ...
        for week_pos in range(0, self.plan.weeks_nb):
            ...
            for day_pos in range(0, 7):
                ...
                for session in self.plan.sessions.filter(
                        week=week_pos, day=day_pos):
                    content = '<b><font color=lightsteelblue size=9>{}</font></b> <font size=9>{}</font>'.format(
                        session.sport, session.name)
                ...
                if sum_distance or sum_time:
                    ...
                    content = '<font size=8 color=darkslateblue>Total: {}</font>'.format(
                        content)
