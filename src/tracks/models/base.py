...
class Track(models.Model):
    ...
    split_total = models.OneToOneField(
        'tracks.TrackSplit',
        null=True,
        blank=True,
        related_name='direct_track')
    ...
    def attach_session(self, user, identity):
        ...
        sport_week, _ = SportWeek.objects.get_or_create(
            user=user, year=year, week=week)
        day, _ = SportDay.objects.get_or_create(
            date=identity['date'], week=sport_week)
        ...
        if sessions.count():
            ...
            for s in sessions:
                ...
                if s.distance and identity['distance']:
                    ratio_distance = abs(
                        s.distance - identity['distance']) / identity['distance']
    ...
    def build_image_path(self, type):
        ...
        h = md5(
            'track:%s:%s:%d' %
            (type, settings.SECRET_KEY, self.pk)).hexdigest()
    ...
    def build_image(self):
        ...
        # longitude of the left bound, latitude of the top bound, longitude of
        # the right bound, latitude of bottom edge.
        params['bbox'] = '%.4f,%.4f,%.4f,%.4f' % (
            min_lon, max_lat, max_lon, min_lat)

        # Add path from simple polyline
        params['paths'] = ','.join(['%.4f,%.4f' %
                                    (lon, lat) for lat, lon in self.simple])
    ...
    def build_thumb(self):
        ...
        box_offset = 0.2
        box = (w * box_offset, h * box_offset, w *
               (1.0 - box_offset), h * (1.0 - box_offset))
