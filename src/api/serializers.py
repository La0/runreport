...
class PlanSessionSerializer(serializers.ModelSerializer):
    sport = serializers.PrimaryKeyRelatedField(
        queryset=Sport.objects.filter(depth=1))

    class Meta:
        model = PlanSession
        fields = ('id', 'week', 'day', 'name', 'comment', 'sport',
                  'type', 'distance', 'time', 'place', 'hour')
...
class PlanSerializer(serializers.ModelSerializer):
    ...
    sessions_nb = serializers.IntegerField(
        source='sessions.count', read_only=True)
    export_pdf = serializers.HyperlinkedIdentityField(
        view_name='plan-export-pdf')
    applications_nb = serializers.IntegerField(
        source='applications.count', read_only=True)
    is_active = serializers.BooleanField(read_only=True)

    class Meta:
        model = Plan
        fields = ('id', 'name', 'weeks_nb', 'sessions_nb', 'updated',
                  'start', 'end', 'export_pdf', 'applications_nb', 'is_active', )
...
class ClubMembershipSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ClubMembership
        fields = ('id', 'role', 'created', 'club',
                  'groups', 'athletes', 'places', )
