...
class JsonResponseMixin(object):
    ...
    def render_to_response(self, context):
        if JSON_OPTION_RAW in self.json_options:
            ...
            return HttpResponse(self.jsonify(context),
        ...
        if JSON_OPTION_ONLY_AJAX in self.json_options:
            # Skip when not Ajax
            if not ajax:
                # Render template in full page
                context['base'] = 'modal.base.html'
                return super(JsonResponseMixin,
    ...
    def build_response(self, html=None, message=None, url=None, output=None):
        ...
        if output is not None:
            data['output'] = output
        return HttpResponse(self.jsonify(
    ...
    def dispatch(self, *args, **kwargs):
        ...
        # Catch redirection
        if isinstance(
                resp, HttpResponseRedirect) and JSON_OPTION_REDIRECT_SKIP not in self.json_options:
...
class CsvResponseMixin(object):
    ...
    def render_to_response(self, context):
        ...
        response['Content-Disposition'] = 'attachment; filename="%s.csv"' % context.get(
            'csv_filename', 'output')
        ...
        if 'csv_headers' in context:
            ...
            writer = csv.DictWriter(
                response,
                fieldnames=context['csv_headers'],
                delimiter=';',
                dialect='excel')
            writer.writeheader()
