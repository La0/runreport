...
urlpatterns = [
    ...
    # Invite
    url(r'^invite/(?P<slug>.*)',
        ClubInviteCheck.as_view(), name="club-invite"),

    # Landing pages
    url(r'^features/athlete/?',
        TemplateView.as_view(
            template_name='landing/athlete.html'),
        name="features-athlete"),
    url(r'^features/trainer/?',
        TemplateView.as_view(
            template_name='landing/trainer.html'),
        name="features-trainer"),
    url(r'^legal/(?P<type>mentions|cgu)/?',
        LegalView.as_view(), name="legal"),

    # Contact Form
    url(r'^contact/(?P<sent>sent)?',
...
prod_urls = [
    url(r'^%s/' %
        settings.ADMIN_BASE_URL, include(admin.site.urls)),
    url(
        r'^admin/',
        RedirectView.as_view(
            url='http://docs.djangoproject.com/en/dev/ref/contrib/admin/',
            permanent=True)),
]
