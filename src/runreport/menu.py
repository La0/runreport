...
def add_pages(request):
    ...
    if request.user.is_authenticated():
        ...
        # Load memberships
        members = request.user.memberships.exclude(
            role__in=('archive', 'prospect'))
        ...
        for m in members:
            ...
            if m.role in ('athlete', ):
                ...
                submenu['menu'].append(
                    _p(('club-groups', m.club.slug, ), _('Groups'), lazy=True))
            ...
            if m.role in ('trainer', 'staff') or request.user.is_superuser:
                submenu['menu'].append(
                    _p(('club-members-name', m.club.slug, 'athletes', 'name'), _('My athletes')))
                submenu['menu'].append(
                    _p(('club-races', m.club.slug, ), _('Races')))
                submenu['menu'].append(
                    _p(('club-groups', m.club.slug, ), _('Groups'), lazy=True))
                submenu['menu'].append(
                    _p(('club-members-name', m.club.slug, 'all', 'name'), _('All the club')))

                # Manage links
                if m.club.manager == request.user or request.user.is_superuser:
                    submenu['menu'].append(
                        _p(('club-members-name', m.club.slug, 'prospects', 'name'), _('Newcomers')))
                    submenu['menu'].append(
                        _p(('club-members-name', m.club.slug, 'archives', 'name'), _('Archives')))
                    submenu['menu'].append(
                        _p(('places', m.club.slug, ), _('Places'), lazy=True))
                    submenu['menu'].append(
        ...
        submenu['menu'].append(
            _p(('user-public-profile', request.user.username), _('My public profile')))
        ...
        submenu['menu'].append(_p('gcal-oauth', _('My Google Calendar')))
