...
class MessageOwned(object):
    ...
    def get_object(self):
        return get_object_or_404(
            Message, pk=self.kwargs['message_id'], writer=self.request.user)
...
class ConversationMixin(object):
    ...
    def get_conversation(self):
        ...
        if self.conversation.type in (
                TYPE_COMMENTS_PRIVATE, TYPE_COMMENTS_PUBLIC):
            # A comment conversation has rights
            self.session = self.conversation.get_session()
            if not self.session:
                raise PermissionDenied
            self.privacy = self.session.day.week.user.get_privacy_rights(
                self.request.user)
            ...
            # A comment conversation has rights
            self.privacy = self.conversation.week.user.get_privacy_rights(
                self.request.user)
            if self.conversation.type not in self.privacy:
                raise PermissionDenied
...
class MessagePostMixin(object):
    ...
    def get_post(self):
        ...
        self.post = get_object_or_404(
            Post, pk=self.kwargs['post_id'], published=True)
...
class MessageSessionMixin(object):
    ...
    def get_session(self):
        ...
        self.session = get_object_or_404(
            SportSession, pk=self.kwargs['session_id'])
