...
class Conversation(models.Model):
    ...
    mail_recipient = models.ForeignKey(
        Athlete,
        null=True,
        blank=True,
        related_name='mail_conversations')
    session_user = models.ForeignKey(
        Athlete,
        null=True,
        blank=True,
        related_name='session_conversations')
    ...
    def get_absolute_url(self):
        ...
        if self.type == TYPE_COMMENTS_WEEK:
            return reverse(
                'user-calendar-week', args=(self.week.user.username, self.week.year, self.week.week))

        if self.type == TYPE_POST:
            return reverse('post', args=(
                self.post.writer.username, self.post.slug, ))
        ...
        if not session:
            return reverse('conversation-view', args=(self.pk, ))
        dt = session.day.date
        return reverse('user-calendar-day',
                       args=(session.day.week.user.username, dt.year, dt.month, dt.day))

    def get_session(self):
        if self.type not in (TYPE_COMMENTS_PRIVATE, TYPE_COMMENTS_PUBLIC, ):
            raise Exception(
                "No session on conversation typed : %s" %
                self.type)
    ...
    def get_recipients(self, exclude=None):
        ...
        '''
        writers = [
            m.writer for m in self.messages.exclude(
                writer=exclude).distinct('writer')]
        if self.type == TYPE_MAIL:
...
class Message(models.Model):
    ...
    def copy(self, conversation):
        ...
        msg, _ = conversation.messages.get_or_create(
            writer=self.writer, message=self.message)

        return msg

        # Check there is not already the same message
        if conversation.messages.filter(
                writer=self.writer, message=self.message).count() > 0:
            return False
